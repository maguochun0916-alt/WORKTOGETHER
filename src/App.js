...
export default function App() {
      ...
      <div className="p-4 sm:p-6 space-y-6 pb-24">
        ...
        <div className="bg-gradient-to-br from-slate-800 via-slate-800 to-slate-900 rounded-[2rem] p-7 text-white shadow-lg relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-xs text-blue-200 font-bold mb-1 tracking-wider uppercase">
              本月排班狀態
            </p>
            <h2
              className={`text-2xl sm:text-3xl font-black tracking-wide leading-tight ${
                userData?.points < 0 ? "text-red-400" : "text-white"
              }`}
            >
        ...
        <div>
          <h2 className="text-lg font-black mb-4 flex items-center gap-2 text-slate-800 tracking-wide px-1">
            <ClipboardList size={22} className="text-blue-500" /> 需求佈告欄{" "}
            ...
            {openOrders.length === 0 ? (
              ...
              openOrders.map((order) => {
                ...
                return (
                  <div
                    key={order.id}
                    className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden flex flex-col"
                  >
                    <div
                      ...
                      <div className="flex flex-col items-end gap-3">
                        {canManage && (
                          <button
                            ...
                            className="p-2 -mt-1 -mr-1 text-slate-300 hover:text-rose-500 active:scale-90 transition-all bg-slate-50 hover:bg-rose-50 rounded-full"
                          >
                            <X size={16} />
                          </button>
                        )}
                        <button
                          ...
                          className={`text-lg font-black tracking-tight ${
                            canManage
                              ? "active:scale-95 hover:text-blue-600 text-slate-800"
                              : "text-slate-800"
                          }`}
                        >
                          💰 {displayAmt}
      ...
      <div className="p-4 sm:p-6 pb-24 space-y-5">
        ...
        {matched.map((order) => {
          ...
          return (
            <div
              key={order.id}
              className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm relative"
            >
              <div className="flex justify-between items-center mb-5">
                ...
                {canManage && (
                  <button
                    onClick={() => deleteOrder(order.id)}
                    className="p-1.5 -mr-1.5 text-slate-300 hover:text-rose-500 active:scale-90 transition-all rounded-full"
                  >
                    <X size={18} />
                  </button>
                )}
              ...
              <div className="flex items-center justify-between text-center bg-slate-50 p-4 rounded-2xl border border-slate-100/50">
                <div className="flex-1">
                  <p className="text-[11px] text-slate-400 font-bold tracking-widest mb-1">
                    休假方
                  </p>
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] max-w-md mx-auto relative font-sans overflow-x-hidden selection:bg-blue-100">
      ...
      {msg && (
        <div
          className={`fixed top-6 left-1/2 -translate-x-1/2 z-[100] px-6 py-3.5 rounded-full text-white text-sm font-bold shadow-lg animate-in fade-in slide-in-from-top-4 flex items-center gap-2 ${
            msg.type === "error"
              ? "bg-rose-600"
              : msg.type === "warning"
              ? "bg-amber-500"
              ...
              {["A", "B", "C"].map((g) => (
                <button
                  ...
                  className={`flex-1 py-3 text-sm font-black rounded-xl transition-all touch-manipulation ${
                    loginGroup === g
                      ? "bg-white shadow-sm border border-slate-200/60 text-blue-600"
                      : "text-slate-400"
                  }`}
                >
                  {g}股
            ...
            >
              <div>
                ...
                <input
                  ...
                  className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-slate-700"
                />
              </div>
              <div>
                <label className="text-xs font-bold text-slate-500 ml-1 mb-2 block tracking-wider">
                ...
                <select
                  ...
                  className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none appearance-none text-slate-700"
                >
      ...
      <header className="sticky top-0 bg-white/80 backdrop-blur-xl p-5 flex justify-between items-center z-40 border-b border-slate-100">
        <h1 className="font-black text-slate-800 tracking-widest text-lg">
          {SITE_NAME}
        </h1>
        <button
          onClick={() => {
          ...
          className="p-2 bg-slate-50 text-slate-400 rounded-full hover:text-slate-700 active:scale-90 transition-all border border-slate-100"
        >
      ...
      <nav className="fixed bottom-0 w-full max-w-md bg-white/80 backdrop-blur-xl border-t border-slate-200/50 pt-2 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] px-2 flex justify-around items-center z-40 shadow-[0_-10px_30px_-15px_rgba(0,0,0,0.05)]">
        <button
          ...
          className={`flex-1 py-2 flex flex-col items-center transition-all touch-manipulation active:scale-[0.98] ${
            activeTab === "home" ? "text-blue-600" : "text-slate-400"
          }`}
        >
          <div
            className={`p-1.5 rounded-xl mb-0.5 ${
              activeTab === "home" ? "bg-blue-50" : "bg-transparent"
            }`}
          >
        ...
        >
          <div
            className={`p-1.5 rounded-xl mb-0.5 ${
              activeTab === "completed" ? "bg-blue-50" : "bg-transparent"
            }`}
          >
            <CheckCircle
              size={22}
